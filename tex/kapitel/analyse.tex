...
\label{Analyse:OPCUA}
Die Kommunikation zwischen \ac{OPC UA} Komponenten findet auf der Anwendungsschicht des \ac{TCP}/\ac{IP} Referenzmodells statt. \ac{OPC UA} stellt eine vertikale Verbindung der Kommunikationspartner auf allen Ebenen der Automatisierungspyramide her. Aufgrund der Öffnung von Unternehmen nach außen, den immer höheren Informationsbedarf und die direkte Kommunikation mit Händler und Kunden ist \ac{OPC UA} ein attraktives Ziel für Industriespionage und Sabotage des Netzwerks (\cite{opcuapt2}). 

\ac{OPC UA} wurde als Client-Server Architektur entwickelt.
TODO - PublishSubscribe
...
• Servers representing services or devices can stream data to applications hosted in the cloud. For example, backend servers, big data analytics for system optimization and predictive maintenance. PubSub is not bound to a particular messaging system. Rather it can be mapped to various different systems as illustrated with two examples:
...
Um eine Unterstützung unterschiedlichster Systeme zu erreichen, stellt \ac{OPC UA} verschiedene Protokolle für unterschiedliche Anforderungen zur Kommunikation zwischen den Komponenten bereit. Das \ac{OPC UA} Binary Protokoll wird für Kommunikation mit optimierter Geschwindigkeit und Durchsatz genutzt. Des Weiteren kann die Kommunikation auch auf Basis von Webservices und den Protokollen \ac{HTTP} und \ac{SOAP} sowie in einer hybriden Form beider durchgeführt werden.
...
Das \ac{UACP} beschreibt ein abstraktes Protokoll zur Herstellung einer Vollduplexverbindung in einer Client-Server Architektur. Implementierungen dieses Protokolls können über jede Middleware, welche den Austausch von Nachrichten im Vollduplexverfahren über \ac{TCP}/\ac{IP} und \textit{Websockets} unterstützt, durchgeführt werden. Somit ist das von \ac{OPC UA} genutzte Anwendungsprotokoll für die Zukunft flexibel. Die Spezifikation des abstrakten Protokolls \ac{UACP} wird im 5. Teil der \ac{OPC UA} Spezifikation\footnote{OPC Unified Architecture Specification Part 6: Mappings (\cite{opcpt5})} beschrieben und beinhaltet die Form der Nachricht, den Verbindungsaufbau, die Kommunikation und die Fehlerbehandlung.

Die beschriebenen Protokolle bilden mit ihren Sicherheitsmechanismen die Grundlage für eine sichere Datenübertragung. Die Form der Datenübertragung über \ac{SOAP}/\ac{HTTP} wurde ab Version 1.03 der Spezifikation als veraltet angesehen, da es in der Industrie nicht umgesetzt wurde (\cite{opcpt5}). Die Protokolle \ac{OPC UA} \ac{TCP} und die Hybridform aus den Protokollen \ac{OPC UA} und \ac{HTTPS} werden im folgenden auf ihre Standhaftigkeit bzgl. der Sicherheitsanforderungen in Industrie 4.0 Umgebungen analysiert.

\subsubsection{\ac{OPC UA} \ac{TCP}}


\subsubsection{\ac{OPC UA} \ac{HTTPS}}
TODO - Ansatz: secure mode in OPCUA


\subsubsection{Konfiguration}
\ac{OPC UA} stellt aufgrund der verschiedenen Anforderungen der Industrie 4.0 Systeme mehrere Sicherheitskonfigurationen zur Informationsübertragung zur Verfügung. Dies ist möglich, da die Verwendung von Verschlüsselungsalgorithmen und Kodierungsverfahren Ressourcen benötigt und Latenzen verursacht, welche unter Umständen nicht vorhanden sind oder nicht geleistet werden können. Eine Fehlkonfiguration des Protokolls kann erhebliche Auswirkungen auf den Betrieb des Netzwerks und der Assets haben sowie die Integrität der Daten gefährden.

Mit Hilfe des vorhandenen Testsystems wurden verschiedene Szenarien der Fehlkonfiguration der \ac{OPC UA} Architektur durchgeführt und die Auswirkungen auf die Kommunikation im Netzwerk untersucht. Die Implementierung der benötigten Komponenten wird in \autoref{Impl:Fehlkonfiguration} beschrieben.

TODO - Ergebnisse der Fehlkonfiguration SecurityMode

\subsubsection{Weitere Sicherheitsbestandteile}
TODO - PKI, Zertifikatsmanagement, Zeitsynchronisation, Kerberos, OAuth2, 
